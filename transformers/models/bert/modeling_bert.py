...
class BertAdapter(nn.Module):
    def __init__(self, config, inputdim, bottleneck, condition):
        super().__init__()
...
class BertOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        if self.condition == 'serial':
            hidden_states = self.dense(hidden_states)
            hidden_states = self.dropout(hidden_states)
            hidden_states = hidden_states + self.adapter_serial(hidden_states)
            hidden_states = self.LayerNorm(hidden_states + input_tensor)
        elif self.condition == 'parallel':
            preserve = hidden_states
            hidden_states = self.dense(hidden_states)
            hidden_states = self.dropout(hidden_states)
            ...
            if 'vector' in args.output_path:
                ...
                source_1 = hidden_states
                hidden_states = self.dense(hidden_states)
                hidden_states = self.dropout(hidden_states)
                control_serial = self.adapter_serial(hidden_states)
