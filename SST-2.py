...
max_len = args.sst_len
batch_size = args.sst_batch
lr = args.sst_lr
...
df_train.columns = ['sen','label']

val_path = os.path.join(data_dir, 'SST-2/dev.tsv')
df_val = pd.read_csv(val_path, sep='\t')
df_val.columns = ['sen','label']

test_path = os.path.join(data_dir, 'SST-2/test.tsv')
...
print('Start training SST-2!!!')
best_acc = 0
best_epoch=0
accuracy = []
for epoch in range(args.sst_epoch):
    ...
    if score >= best_acc:
        best_acc = score
        best_epoch = epoch
        torch.save(model.state_dict(), os.path.join(model_path, 'SST-2.ckpt'))
    end = time.time()
    print('epoch = ', epoch+1)
    ...
    if epoch == 0:
        print('預計train時間 = ', args.sst_epoch*(end-epoch_start)/60, '分鐘')
    print('=====================================')
print('Start predict SST-2!!!')

model = Model(backbond).to(device)
ckpt = torch.load(os.path.join(model_path, 'SST-2.ckpt'))
...
with open(os.path.join(pred_path, 'SST-2.tsv'), 'wt') as out_file:
